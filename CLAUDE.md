...
> Dernière mise à jour : 12 Mars 2026 — v2.0
> Déployé et opérationnel sur Netlify
...
**Organisation** : AgenticX5
**Production** : process-safety-management-x5.netlify.app
...
**SINGLE-FILE HTML.** Le Hub PSM-X5 vit dans `index.html` (~251 KB). Pas de build step, pas de bundler, pas de framework. HTML5 + CSS3 + JavaScript ES6+ inline. Déploiement Netlify CDN statique.
...
- Chart.js v4.4.1 — 9 charts (radar 14 éléments, line SIL trends, 7 charts OSHA)
...
```
PSM-X5/
├── index.html                    # 251KB — Dashboard 6D + 3 Claude API + 7 OSHA charts
├── CLAUDE.md                     # Ce fichier — mémoire projet
├── psm-x5-osha-dataset.json     # 58KB — Données OSHA réelles 2016-2025 + CSB
├── docs/
│   └── PSM-X5_Descriptif_v1.docx # 26KB — Descriptif projet Word
└── .skills/psm-x5-skills/       # 10 skills PSM (42.7KB total)
    ├── psm-hazop-lopa-analysis/SKILL.md
    ├── psm-sis-sif-sil-verification/SKILL.md
    ├── psm-moc-pssr-workflow/SKILL.md
    ├── psm-mechanical-integrity-ragagep/SKILL.md
    ├── psm-incident-investigation-rca/SKILL.md
    ├── psm-emergency-response-planning/SKILL.md
    ├── psm-process-safety-information/SKILL.md
    ├── psm-osha-29cfr1910119-compliance/SKILL.md
    ├── psm-epa-rmp-seveso-comah/SKILL.md
    └── psm-atex-hazardous-areas/SKILL.md
```

## Structure des 6 Dimensions (index.html)

```
Hub Landing ─── Logo AgenticX5, 4 KPIs animés, intro PSM
│
...
│   └── ★ HAZOP IA assisté (Claude API — A3-PHA-Engine) ✅ ACTIF
│
...
│   └── ★ Pack Évidence IA (Claude API — A13-Audit) ✅ ACTIF
│
├── D5 — Agents & Écosystème (accent #e94560)
│   ├── Pipeline 14 agents + 14 cartes détail
│   ├── 12 interconnexions AgenticX5
│   └── ★ Investigation Incident IA (Claude API — A11-RCA) ✅ ACTIF
│
└── D6 — OSHA Data Analytics (accent #e94560) ★ NOUVEAU v1.0
    ├── 4 KPIs (total citations, pénalités, incidents CSB, top élément)
    ├── Chart 1 — Citations par élément OSHA (line multi-année)
    ├── Chart 2 — Tendance pénalités $ (bar chart)
    ├── Chart 3 — API 754 Tier distribution (doughnut)
    ├── Chart 4 — Mechanical Integrity sous-sections (bar horizontal)
    ├── Chart 5 — Timeline incidents CSB réels
    ├── Chart 6 — Top High Hazard Chemicals (bar chart)
    └── Chart 7 — Citations par état US top 10 (bar chart)
```
...
## Claude API — 3 Fonctions IA ✅ OPÉRATIONNELLES
...
Toutes passent par `callClaudeAPI()` → POST api.anthropic.com/v1/messages → `formatAIResponse()` (MD→HTML).

## OSHA Dataset — psm-x5-osha-dataset.json (58KB)

Données réelles OSHA 2016-2025 + incidents CSB :

| Section | Contenu |
|---------|---------|
| `citations_by_element` | Citations par élément PSM par année (2016-2025) |
| `penalties` | Montants pénalités par année |
| `api_754` | Distribution incidents Tier 1-4 |
| `mechanical_integrity` | Breakdown sous-sections MI |
| `csb_incidents` | Timeline incidents CSB réels avec détails |
| `hhc_chemicals` | Top High Hazard Chemicals par fréquence |
| `citations_by_state` | Top 10 états US par nombre de citations |

Chargé async au init via `loadOSHA()`. Fallback gracieux si fichier absent (warning console).

## JavaScript — Fonctions Clés (32 fonctions)

### Navigation & Rendu
```javascript
go(dim)              // Navigation hub/d1-d6
setLang(l)           // Toggle FR/EN
renderCurrentDim()   // Render dimension active
renderHub()          // 4 KPIs animés + count-up
animCount(id,t,d)    // Animation compteur
```
...
### D2: MOC & PSSR
```javascript
renderMOC()          // Timeline 8 étapes
startMOC()           // Simulation live animée
renderPSSR()         // Checklist synchro MOC
```

### D3: SIS/SIL
```javascript
renderSIS()          // 6 SIF cards + gauges SVG
renderSILChart()     // Chart.js tendance PFD 90j
```
...
renderAuditRadar()   // Chart.js radar
```
...
### Claude API
```javascript
callClaudeAPI()      // Appel API générique (POST fetch)
formatAIResponse()   // MD → HTML
generateAIHazop()    // D1 — HAZOP assisté
generateAIAudit()    // D4 — Pack évidence
generateAIRCA()      // D5 — Investigation incident
```

### D6: OSHA Analytics (7 charts)
```javascript
loadOSHA()           // Charge dataset JSON async
renderOSHA()         // Orchestrateur 7 charts
renderOshaKpis()     // 4 KPIs calculés
renderOshaElemChart()    // Citations par élément (line)
renderOshaPenaltyChart() // Pénalités $ (bar)
renderOshaApi754Chart()  // API 754 Tier (doughnut)
renderOshaMiChart()      // MI sous-sections (bar horiz.)
renderCsbTimeline()      // Incidents CSB (timeline)
renderOshaHhcChart()     // Top HHC (bar)
renderOshaStateChart()   // Citations par état (bar)
```

## Données Hardcodées (index.html)

| Variable | Lignes | Contenu |
|----------|--------|---------|
| `PSM_ELEMENTS[]` | 547-563 | 14 éléments PSM (nom FR/EN, agent, desc, icône, KPI, standards) |
| `HAZOP_NODES[]` | 564-586 | 3 noeuds (Réacteur, Colonne, Stockage) — déviations détaillées |
| `MOC_STEPS[]` | 587-597 | 8 étapes MOC avec agents et tags |
| `SIF_DATA[]` | 598-606 | 6 SIF (PFD, target SIL, trend, status) |
| `ECO_LINKS[]` | 607-620 | 12 plateformes AgenticX5 interconnectées |
| `PSM_SYSTEM_HAZOP` | 856-865 | System prompt agent A3-PHA-Engine |
| `PSM_SYSTEM_AUDIT` | 867-876 | System prompt agent A13-Audit |
| `PSM_SYSTEM_RCA` | 877-887 | System prompt agent A11-RCA |
...
## Interconnexions Écosystème AgenticX5 (12)

| Plateforme | Rôle |
|------------|------|
| EDGY-AgenticX5 | Orchestrateur central |
| SafetyGraph | Neo4j 22M+ incidents |
| SENTINEL·X5 | Alarm Intelligence |
| SafeScan360 | Computer Vision |
| SafeTwinX5 | Jumeaux numériques |
| DensityGuard | Mining Process |
| BehaviorX | Analyse comportementale |
| LiteraCIA | Formation adaptative |
| HolisticErgo | Ergonomie postes |
| SafeVision SST | Vidéos formation IA |
| Hovermap Safety | LiDAR Drones ATEX |
| Standards-Gouvernance | Règles opérationnelles |
...
9. **Branding** — AgenticX5 uniquement. Aucune mention genaisafety.online, GitHub, ou anciens noms

## Roadmap

| Version | Statut | Feature |
|---------|--------|---------|
| v1.0 | ✅ Déployé | Dashboard 6D, 14 agents, 3 Claude API actives, 10 skills, 7 charts OSHA, dataset réel, Netlify |
...
```powershell
# Git push → Netlify auto-deploy ~15s (PowerShell)
git add .
...
|------|---------|------------|
| 11 Mars 2026 | v1.0 | Dashboard 5D initial, 14 agents, 10 skills, GitHub+Netlify |
| 11 Mars 2026 | v1.0+ | Ajout logo AgenticX5 base64, copyright, formulaire contact Netlify |
| 11 Mars 2026 | v1.0+ | Ajout D6 OSHA Data Analytics (7 charts), dataset psm-x5-osha-dataset.json |
| 11 Mars 2026 | v1.0+ | Intégration 3 Claude API (HAZOP, Audit, RCA) — opérationnelles |
| 12 Mars 2026 | v1.0+ | Nettoyage branding : suppression genaisafety.online, GitHub, anciens noms |
| 12 Mars 2026 | v2.0 | CLAUDE.md mis à jour — reflète état actuel 6D + 3 API + OSHA dataset |

---
*PSM-X5 — Plateforme #26 AgenticX5 — team@agenticx5.com*
